...
app.post('/submit-insurance', async (req, res) => { try { const newEnquiry = new Insurance(req.body); await newEnquiry.save(); res.json({ success: true, message: "Jankari mil gayi!" }); } catch (error) { res.json({ success: false }); } });
...
// ==========================================
// 4. NAYA: Affiliate Links Routes
// ==========================================
app.post('/add-affiliate', async (req, res) => { try { await new Affiliate({title: req.body.title, url: req.body.url}).save(); res.json({success: true, message: "Link add ho gaya!"}); } catch (err) { res.json({success: false}); } });
...
app.post('/delete-affiliate', async (req, res) => { try { await Affiliate.findByIdAndDelete(req.body.id); res.json({success: true, message: "Link hata diya gaya!"}); } catch (err) { res.json({success: false}); } });
